...
from pycaching.cache import Cache, Type, Size, Waypoint, Country, CountryClassAmbiguityValue
...
class TestMethods(NetworkedTest):
    ...
    def test_cache_country(self):
        with self.subTest(""):
            with self.recorder.use_cassette('cache_type_gigaevent'):
                cache = self.gc.get_cache('GC7WWWW')

                # TODO country with comma inside
                self.assertEqual(cache.country, 'Czechia')


    def test_cache_province(self): ## TODO
        with self.subTest(""):
            with self.recorder.use_cassette('cache_type_gigaevent'):
                cache = self.gc.get_cache('GC7WWWW')

                self.assertEqual(cache.province, 'Hlavní město Praha')
...
class TestLocationProperties(unittest.TestCase):
    ...
    def test_unique_country(self):
        a = []
        for country_id, country_name in Country.countries().items():
            self.assertNotIn(country_name.lower(), a, f'{country_name} is not unique')
            a.append(country_name.lower())

    def test_unique_state(self):
        a = []
        for country_id, country_name in Country.states().items():
            if country_name.lower() in ('distrito federal', 'limburg'):
                continue
            self.assertNotIn(country_name.lower(), a, f'{country_name} is not unique')
            a.append(country_name.lower())
    ...
    def test_from_string(self):
        ...
        with self.subTest('both state, country'):
            c = Country.from_string('bAyErN , Germany')
            self.assertEqual(c.country_id, 79)
            self.assertEqual(c.state_id, 136)
            self.assertEqual(c.country_name, 'Germany')
            self.assertEqual(c.state_name, 'Bayern')
        ...
        with self.subTest('both country, state'):
            c = Country.from_string('Germany, bAyErN')
            self.assertEqual(c.country_id, 79)
            self.assertEqual(c.state_id, 136)
            self.assertEqual(c.country_name, 'Germany')
            self.assertEqual(c.state_name, 'Bayern')
        ...
        with self.subTest('Bonaire, Sint Eustatius and Saba'):
            c = Country.from_string('Bonaire, Sint Eustatius and Saba')
        ...
        with self.subTest('Bonaire'):
            c = Country.from_string('Bonaire')
            self.assertEqual(c.country_id, 279)
            self.assertEqual(c.state_id, 523)
            self.assertTrue(c.has_state)

        with self.subTest('Sint Eustatius'):
            c = Country.from_string('Sint Eustatius')
            self.assertEqual(c.country_id, 279)
            self.assertEqual(c.state_id, 524)
            self.assertTrue(c.has_state)

        with self.subTest('Saba'):
            c = Country.from_string('Saba')
            self.assertEqual(c.country_id, 279)
            self.assertEqual(c.state_id, 525)
            self.assertTrue(c.has_state)

        with self.subTest('Sint Eustatius, Bonaire, Sint Eustatius and Saba'):  # GC4NDTH
            c = Country.from_string('Sint Eustatius, Bonaire, Sint Eustatius and Saba')
            self.assertEqual(c.country_id, 279)
            self.assertEqual(c.state_id, 524)
            self.assertTrue(c.has_state)

    def test_all_country(self):
        for country_id, name in Country.countries().items():
            with self.subTest(name):
                c = Country.from_string(name)
                self.assertEqual(c.country_id, country_id)

    def test_all_state(self):
        for state_id, name in Country.states().items():
            if name == 'Georgia' or name == 'Luxembourg':  # State name is a country name, too
                continue
            if name in ('Limburg','Distrito Federal'):  # Names are not ambiguity
                continue
    ...
    def test_ambiguity_Limburg(self):
        ...
        with self.subTest(f'from_string({name})'):
            try:
                c = Country.from_string(name)
                self.assertTrue(False)
            except CountryClassAmbiguityValue:
                pass

        with self.subTest(f'from_string_state({name})'):
            try:
                c = Country.from_string_state(name)
                self.assertTrue(False)
            except CountryClassAmbiguityValue:
                pass
    ...
    def test_special_names(self):
        ...
        c = Country.from_string_country('Bonaire, Sint Eustatius and Saba')
        self.assertEqual(c.country_id, 279)
        self.assertIsNone(c.state_id)
        self.assertFalse(c.has_state)
